  ...
  private void configureBindings() {    
    ...
    for (int i = 0; i < cardinalDirectionLockTriggers.length; i++) {
      ...
      trigger.whileTrue(m_driveTrain.applyRequest( // could be better to change whileTrue to onTrue or toggleOnTrue
        ...
        .withVelocityX((-ControllerConstants.driveController.getLeftX()) * m_driveTrain.maxSpeedSupplier.get()) // Drive forward with negative Y (forward)
        .withVelocityY((-ControllerConstants.driveController.getLeftY()) * m_driveTrain.maxSpeedSupplier.get()) // Drive left with negative X (left)
